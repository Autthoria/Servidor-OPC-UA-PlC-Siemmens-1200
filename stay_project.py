...
class Login_Screen(QMainWindow):
    def __init__(self):
        ...
        icon_code = QIcon("C:/Python38/Estadias/imagenes/hash.svg")
        icon_password = QIcon("C:/Python38/Estadias/imagenes/lock.svg")
        self.in_username.addAction(icon_user, QLineEdit.LeadingPosition)
        self.in_employee_code.addAction(icon_code, QLineEdit.LeadingPosition)
    ...
    def Login(self):
        ...
        if len(str(username))==0 or len(str(employee_code))==0 or len(str(password))==0:
            self.label_message.setText("Enter all necessary data.")
        else:
            try:
                con = Connection.connection_database()
                cursor = con.cursor()
                sql = """SELECT Username, Employee_code, Password FROM Users WHERE Username = '{}' 
                AND Employee_code = '{}' AND Password = '{}'""".format(username, employee_code, password)
                cursor.execute(sql)
                if cursor.fetchall():
                    self.access_to_HMI()
                    
                    # Iniciar el temporizador para captura de datos cada 3 minutos (180,000 ms)
                    self.timer = QTimer(self)
                    self.timer.timeout.connect(lambda: self.capture_and_warning(username, employee_code))
                    ...
                    msg_boxes.incorrect_msgbox('ERROR', 'The credentials entered are incorrect. Try again.')
            
            except mysql.connector.Error as error:
                print("Error connecting to database {}".format(error))
    
    def capture_and_warning(self, username, employee_code):
        global new_value1, new_value2, new_value3, new_value4
        
        date_time = datetime.now()
        self.wrnings = HMI()
        z = self.wrnings.warning()  # Ejecutar warning con los valores actualizados
        self.data = Database_Interface()
        self.data.capture_information(date_time, username, employee_code, new_value1,
                                      new_value2, new_value3, new_value4, z)
...
class Forgotten_Account(QMainWindow):
    ...
    def recover_password(self):
        ...
        con = Connection.connection_database()
        cursor = con.cursor()
        sql = "SELECT * FROM Users WHERE Employee_code = '{}'".format(employee_code_to_search)
        cursor.execute(sql)
...
class User_Form(QMainWindow):
    ...
    def create_user_table(self):
        con = Connection.connection_database()
        cursor = con.cursor()
        sql = """CREATE TABLE IF NOT EXISTS Users (Name VARCHAR(255), Middle_name VARCHAR(255), Last_name VARCHAR(255), 
        Username VARCHAR(255), Age INT, Gender VARCHAR(255), Phone_number INT, Email VARCHAR(255), 
        Employee_code VARCHAR(255), Password VARCHAR(255))"""
        cursor.execute(sql)
        con.commit()
        con.close()
...
class HMI(QMainWindow):
    def __init__(self):
        ...
        # Inicializar valores antiguos para detectar cambios
        self.previous_value1 = None
        self.previous_value2 = None
        self.previous_value3 = None
        self.previous_value4 = None
    ...
    def update_plot1(self):
        global data1, client, new_value1
        try:
            client.connect()
            ...
            if self.previous_value1 != Sensor_1:  # Verificar si el valor ha cambiado
                self.previous_value1 = Sensor_1
                data1 = np.append(data1, Sensor_1)
                self.Num1.display(Sensor_1)
                self.plot_all()
        finally:
            client.disconnect()

    def update_plot2(self):
        global data2, client, new_value2
        try:
            client.connect() 
            ...
            if self.previous_value2 != Sensor_2:
                self.previous_value2 = Sensor_2
                data2 = np.append(data2, Sensor_2)
                self.Num2.display(Sensor_2)
                self.plot_all()
        finally:
            client.disconnect()

    def update_plot3(self):
        global data3, client, new_value3
        try:
            client.connect()
            ...
            if self.previous_value3 != Sensor_3:
                self.previous_value3 = Sensor_3
                data3 = np.append(data3, Sensor_3)
                self.Num3.display(Sensor_3)
                self.plot_all()
        finally:
            client.disconnect()
        
    def update_plot4(self):
        global data4,client, new_value4
        try:
            client.connect()
            ...
            if self.previous_value4 != Sensor_4:
                self.previous_value4 = Sensor_4
                data4 = np.append(data4, Sensor_4)
                self.Num4.display(Sensor_4)
                self.plot_all()
        finally:
            client.disconnect()
    ...
    def plot_all(self):
        self.graphWidget.clear()  # Limpiar el gráfico anterior
    ...
    def warning(self):
        ...
        if new_value1 > 50:
            warnings.append("Sensor 1 value is out of range.")
        if new_value2 > 50:
            warnings.append("Sensor 2 value is out of range.")
        if new_value3 > 50:
            warnings.append("Sensor 3 value is out of range.")
        if new_value4 > 50:
            warnings.append("Sensor 4 value is out of range.")
        
        if warnings:
            observation = "Alert! " + " ".join(warnings) + " Immediate inspection is required."
        else:
            observation = "The value of the sensors is within the allowable range"
...
class Database_Interface(QMainWindow):
    def __init__(self):
        ...
        self.bt_update_database.clicked.connect(self.show_information_in_table)
    ...
    def create_information_table(self):
        conn = Connection.connection_database()
        cursor2 = conn.cursor()
        cursor2.execute("""CREATE TABLE IF NOT EXISTS Information (
                        Date_Time VARCHAR(255), 
                        Username VARCHAR(255),
                        Employee_code VARCHAR(255), 
                        Value_input_1 INT NOT NULL, 
                        Value_input_2 INT NOT NULL, 
                        Value_input_3 INT NOT NULL,
                        Value_input_4 INT NOT NULL, 
    ...
    def show_information_in_table(self):
        info = self.show_database_information()
        i = len(info)
        self.table_information.setRowCount(i)
        tablerow = 0
        for row in info:
            #self.Id = row[0]
            self.table_information.setItem(tablerow,0,QtWidgets.QTableWidgetItem(str(row[0])))
            self.table_information.setItem(tablerow,1,QtWidgets.QTableWidgetItem(str(row[1])))
            self.table_information.setItem(tablerow,2,QtWidgets.QTableWidgetItem(str(row[2])))
            self.table_information.setItem(tablerow,3,QtWidgets.QTableWidgetItem(str(row[3])))
            self.table_information.setItem(tablerow,4,QtWidgets.QTableWidgetItem(str(row[4])))
            self.table_information.setItem(tablerow,5,QtWidgets.QTableWidgetItem(str(row[5])))
            self.table_information.setItem(tablerow,6,QtWidgets.QTableWidgetItem(str(row[6])))
            self.table_information.setItem(tablerow,7,QtWidgets.QTableWidgetItem(str(row[7])))
            tablerow += 1
